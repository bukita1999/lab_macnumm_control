...
class Car_Controller:
    def __init__(self):
        ...
        self.target_obj1_in_cam2 = np.array([[-0.0137449, -0.99765105, 0.06714326, 0.0174374],
                                        [0.99978385, -0.01266235, 0.01651668, 0.68542393],
                                        [-0.01562745, 0.06735598, 0.99760868, 0.74580226],
                                        [0., 0., 0., 1.]])
        self.target_obj2_in_cam2 = np.array([[-0.09200589, -0.99155748, 0.0913711, 0.05668435],
                                        [0.99564808, -0.09024133, 0.02326795, 0.04420056],
                                        [-0.01482606, 0.09311425, 0.99554504, 0.76173723],
                                        [0., 0., 0., 1.]])
        self.cam1_in_cam2 = np.array([[0.99445312, -0.00209779, 0.10518245, 0.0297208],
    ...
    def control_rot(self):
        ...
        while not rospy.is_shutdown():
            ...
            obj1_in_cam2 = np.dot(self.cam1_in_cam2,obj1_in_cam1)
            target_p1 = np.array([self.target_obj1_in_cam2[0,3], self.target_obj1_in_cam2[1,3]])
            target_p2 = np.array([self.target_obj2_in_cam2[0,3], self.target_obj2_in_cam2[1,3]])
            ...
            self.deg_error.append(theta_deg)
    ...
    def control_left_right(self):
        print("****************************************** start control left right ******************************************")
        ...
        while not rospy.is_shutdown():
            ...
            print('**************')
            self.left_error.append(left_righ_error1)


            if abs(left_righ_error1) > 0.002 or abs(left_righ_error1) > 0.002:  # 先调整旋转角度
                if left_righ_error1 > 0:
                    command = MecanumCommand()
                    command.motion_type = 'right'
                    command.accel_time = 0.08
                    command.cruise_time = 0.1 * abs(left_righ_error1*100)
                    command.decel_time = 0.08
                    command.target_rpm = 50.0
                    self.cmd_pub.publish(command)
                    time.sleep(0.5 +  0.1 * abs(left_righ_error1*100))
                else:
                    command = MecanumCommand()
                    ...
                    command.target_rpm = 50.0
                    self.cmd_pub.publish(command)
                    time.sleep(0.5 +  0.1 * abs(left_righ_error1))
            else:
                break

    def control_forward_back(self):
        print("****************************************** start control_forward_back ******************************************")
        ...
        while not rospy.is_shutdown():
            ...
            f_b_error1 =target_p1[1] - current_p1[1]
            f_b_error2 =target_p2[1] - current_p2[1]
            print(f_b_error1)
            print(f_b_error2)
            print('**************')
            self.forward_error.append(f_b_error1)


            if abs(f_b_error1) > 0.002 or abs(f_b_error2) > 0.002: # 先调整旋转角度
                if f_b_error1 > 0:
                    command = MecanumCommand()
                    command.motion_type = 'forward'
                    command.accel_time = 0.1
                    command.cruise_time = 0.1 * abs(f_b_error1*100)
                    command.decel_time = 0.1
                    command.target_rpm = 50.0
                    self.cmd_pub.publish(command)
                    time.sleep(0.5 +  0.1 * abs(f_b_error1*100))
                else:
                    command = MecanumCommand()
                    command.motion_type = 'backward'
                    command.accel_time = 0.1
                    command.cruise_time = 0.1 * abs(f_b_error1*100)
                    command.decel_time = 0.1
                    command.target_rpm = 50.0
                    self.cmd_pub.publish(command)
                    time.sleep(0.5 +  0.1 * abs(f_b_error1))
            else:
                break
    ...
    def spin(self):
        ...
        while not rospy.is_shutdown():
            self.control_rot()
            self.control_left_right()
            self.control_forward_back()
            self.control_rot()
            self.control_left_right()
            self.control_forward_back()


            np.save("/home/robotlab/chuang/ros_Ws/src/data/deg_error.npy",np.array(self.deg_error))
            np.save("/home/robotlab/chuang/ros_Ws/src/data/left_error.npy",np.array(self.left_error))
            np.save("/home/robotlab/chuang/ros_Ws/src/data/forward_error.npy",np.array(self.forward_error))
            break
            rate.sleep()
